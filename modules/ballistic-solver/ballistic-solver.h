...
class BallisticSolver {
 public:
  ...
  /**
   * @brief 初始化
   * @param start_t 开始时间，单位：s
   * @param end_t 最远截止时间，单位：s
   * @param [in] start_x 初位置，单位：m, m, m
   * @param iter 最大枚举次数
   */
  void Initialize(double start_t, double end_t, CVec REF_IN start_x, size_t iter);
 ...
 private:
  ...
  size_t iter_{};       ///< 最大枚举次数
  double start_t_{};    ///< 开始时间，单位：s
  double end_t_{};      ///< 最大结束时间，单位：s
  CVec start_x_{};      ///< 初始位置，单位：m, m, m
  CVec start_v_{};      ///< 初始速度，单位：m/s, m/s, m/s
  CVec intrinsic_v_{};  ///< 固有速度，单位：m/s, m/s, m/s
